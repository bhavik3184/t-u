...
namespace Nop.Services.SubscriptionOrders
{
    ...
    /// </summary>
    public static class SubscriptionCartExtensions
    {
        /// <summary>
        ...
        public static bool RequiresShipping(this IList<SubscriptionCartItem> borrowCart)
        {
        ...
        public static int GetTotalProducts(this IList<SubscriptionCartItem> borrowCart)
        {
            int result = 0;
            foreach (SubscriptionCartItem sci in borrowCart)
            {
                result += sci.Quantity;
            }
            return result;
        ...
        /// <param name="borrowCart">Shopping cart</param>
        /// <returns>Result</returns>
        public static bool IsRecurring(this IList<SubscriptionCartItem> borrowCart)
        {
            foreach (SubscriptionCartItem sci in borrowCart)
            {
        ...
        public static string GetRecurringCycleInfo(this IList<SubscriptionCartItem> borrowCart,
            ILocalizationService localizationService,
            out int cycleLength, out RecurringPlanCyclePeriod cyclePeriod, out int totalCycles)
        {
            cycleLength = 0;
            cyclePeriod = 0;
            totalCycles = 0;
            ...
            RecurringPlanCyclePeriod? _cyclePeriod = null;
            ...
            foreach (var sci in borrowCart)
            {
                ...
                if (product == null)
                {
                    throw new NopException(string.Format("Product (Id={0}) cannot be loaded", sci.PlanId));
                }

                if (product.IsRecurring)
                {
                    string conflictError = localizationService.GetResource("SubscriptionCart.ConflictingShipmentSchedules");
        ...
        public static Customer GetCustomer(this IList<SubscriptionCartItem> borrowCart)
        {
        ...
        public static IEnumerable<SubscriptionCartItem> LimitPerStore(this IEnumerable<SubscriptionCartItem> cart, int storeId)
        {

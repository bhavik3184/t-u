...
namespace Nop.Plugin.Payments.CCAvenue
{
    ...
    public class CCAvenuePaymentProcessor : BasePlugin, IPaymentMethod
    {
        ...
        public void PostProcessPayment(PostProcessPaymentRequest postProcessPaymentRequest)
        {
            ...
            remotePostHelperData.Add("Amount", postProcessPaymentRequest.SubscriptionOrder.SubscriptionOrderTotal.ToString(new CultureInfo("en-US", false).NumberFormat));
            remotePostHelperData.Add("Currency", _currencyService.GetCurrencyById(_currencySettings.PrimaryStoreCurrencyId).CurrencyCode);
            remotePostHelperData.Add("Order_Id", postProcessPaymentRequest.SubscriptionOrder.Id.ToString());
           ...
           // remotePostHelperData.Add("Checksum", myUtility.getchecksum(_ccAvenuePaymentSettings.MerchantId.ToString(), postProcessPaymentRequest.Order.Id.ToString(), postProcessPaymentRequest.Order.OrderTotal.ToString(), _webHelper.GetStoreLocation(false) + "Plugins/PaymentCCAvenue/Return", _ccAvenuePaymentSettings.Key));
            ...
            //Billing details
            remotePostHelperData.Add("billing_name", postProcessPaymentRequest.SubscriptionOrder.BillingAddress.FirstName);
           // remotePostHelperData.Add("billing_address", postProcessPaymentRequest.Order.BillingAddress.Address1 + " " + postProcessPaymentRequest.Order.BillingAddress.Address2);
            ...
            remotePostHelperData.Add("billing_address", postProcessPaymentRequest.SubscriptionOrder.BillingAddress.Address1);
            remotePostHelperData.Add("billing_tel", postProcessPaymentRequest.SubscriptionOrder.BillingAddress.PhoneNumber);
            remotePostHelperData.Add("billing_email", postProcessPaymentRequest.SubscriptionOrder.BillingAddress.Email);

            remotePostHelperData.Add("billing_city", postProcessPaymentRequest.SubscriptionOrder.BillingAddress.City);
            var billingStateProvince = postProcessPaymentRequest.SubscriptionOrder.BillingAddress.StateProvince;
            if (billingStateProvince != null)
                remotePostHelperData.Add("billing_state", billingStateProvince.Abbreviation);
            else
                remotePostHelperData.Add("billing_state", "");
            remotePostHelperData.Add("billing_zip", postProcessPaymentRequest.SubscriptionOrder.BillingAddress.ZipPostalCode);
            var billingCountry = postProcessPaymentRequest.SubscriptionOrder.BillingAddress.Country;
            ...
            if (postProcessPaymentRequest.SubscriptionOrder.ShippingStatus != ShippingStatus.ShippingNotRequired)
            {
                remotePostHelperData.Add("delivery_name", postProcessPaymentRequest.SubscriptionOrder.ShippingAddress.FirstName);
                ...
                remotePostHelperData.Add("delivery_address", postProcessPaymentRequest.SubscriptionOrder.ShippingAddress.Address1);
                //   remotePostHelper.Add("delivery_cust_notes", string.Empty);
                remotePostHelperData.Add("delivery_tel", postProcessPaymentRequest.SubscriptionOrder.ShippingAddress.PhoneNumber);
                remotePostHelperData.Add("delivery_city", postProcessPaymentRequest.SubscriptionOrder.ShippingAddress.City);
                var deliveryStateProvince = postProcessPaymentRequest.SubscriptionOrder.ShippingAddress.StateProvince;
                ...
                remotePostHelperData.Add("delivery_zip", postProcessPaymentRequest.SubscriptionOrder.ShippingAddress.ZipPostalCode);
                var deliveryCountry = postProcessPaymentRequest.SubscriptionOrder.ShippingAddress.Country;
        ...
        public void PostProcessPaymentOld(PostProcessPaymentRequest postProcessPaymentRequest)
        {
            ...
            remotePostHelper.Add("Amount", postProcessPaymentRequest.SubscriptionOrder.SubscriptionOrderTotal.ToString(new CultureInfo("en-US", false).NumberFormat));
            remotePostHelper.Add("Currency", _currencyService.GetCurrencyById(_currencySettings.PrimaryStoreCurrencyId).CurrencyCode);
            remotePostHelper.Add("Order_Id", postProcessPaymentRequest.SubscriptionOrder.Id.ToString());
            remotePostHelper.Add("Redirect_Url", _webHelper.GetStoreLocation(false) + "Plugins/PaymentCCAvenue/Return");
            remotePostHelper.Add("Checksum", myUtility.getchecksum(_ccAvenuePaymentSettings.MerchantId.ToString(), postProcessPaymentRequest.SubscriptionOrder.Id.ToString(), postProcessPaymentRequest.SubscriptionOrder.SubscriptionOrderTotal.ToString(), _webHelper.GetStoreLocation(false) + "Plugins/PaymentCCAvenue/Return", _ccAvenuePaymentSettings.Key));


            //Billing details
            remotePostHelper.Add("billing_cust_name", postProcessPaymentRequest.SubscriptionOrder.BillingAddress.FirstName);
            remotePostHelper.Add("billing_cust_address", postProcessPaymentRequest.SubscriptionOrder.BillingAddress.Address1);
            remotePostHelper.Add("billing_cust_tel", postProcessPaymentRequest.SubscriptionOrder.BillingAddress.PhoneNumber);
            remotePostHelper.Add("billing_cust_email", postProcessPaymentRequest.SubscriptionOrder.BillingAddress.Email);
            remotePostHelper.Add("billing_cust_city", postProcessPaymentRequest.SubscriptionOrder.BillingAddress.City);
            var billingStateProvince = postProcessPaymentRequest.SubscriptionOrder.BillingAddress.StateProvince;
            if (billingStateProvince != null)
                remotePostHelper.Add("billing_cust_state", billingStateProvince.Abbreviation);
            else
                remotePostHelper.Add("billing_cust_state", "");
            remotePostHelper.Add("billing_zip_code", postProcessPaymentRequest.SubscriptionOrder.BillingAddress.ZipPostalCode);
            var billingCountry = postProcessPaymentRequest.SubscriptionOrder.BillingAddress.Country;
            if (billingCountry != null)
            ...
            if (postProcessPaymentRequest.SubscriptionOrder.ShippingStatus != ShippingStatus.ShippingNotRequired)
            {
                remotePostHelper.Add("delivery_cust_name", postProcessPaymentRequest.SubscriptionOrder.ShippingAddress.FirstName);
                remotePostHelper.Add("delivery_cust_address", postProcessPaymentRequest.SubscriptionOrder.ShippingAddress.Address1);
                remotePostHelper.Add("delivery_cust_notes", string.Empty);
                remotePostHelper.Add("delivery_cust_tel", postProcessPaymentRequest.SubscriptionOrder.ShippingAddress.PhoneNumber);
                remotePostHelper.Add("delivery_cust_city", postProcessPaymentRequest.SubscriptionOrder.ShippingAddress.City);
                var deliveryStateProvince = postProcessPaymentRequest.SubscriptionOrder.ShippingAddress.StateProvince;
                if (deliveryStateProvince != null)
                    remotePostHelper.Add("delivery_cust_state", deliveryStateProvince.Abbreviation);
                else
                    remotePostHelper.Add("delivery_cust_state", "");
                remotePostHelper.Add("delivery_zip_code", postProcessPaymentRequest.SubscriptionOrder.ShippingAddress.ZipPostalCode);
                var deliveryCountry = postProcessPaymentRequest.SubscriptionOrder.ShippingAddress.Country;
                if (deliveryCountry != null)

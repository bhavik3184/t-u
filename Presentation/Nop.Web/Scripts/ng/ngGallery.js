...
.directive('ngGallery', ['$document', '$timeout', '$q', '$templateCache', function($document, $timeout, $q, $templateCache) {
	...
	'    <img ng-src="{{ i.ImageUrl }}" class="{{ thumbClass }}" ng-click="openGallery($index)" alt="Image {{ $index + 1 }}" />' +
	'  </div>' +
	'</div>' +
	...
	'  <img ng-src="{{ FullSizeImageUrl }}" ng-click="nextImage()" ng-show="!loading" class="effect" />' +
	'  <a class="nav-right" ng-click="nextImage()"><i class="fa fa-angle-right"></i></a>' +
	'  <span class="info-text">{{ index + 1 }}/{{ images.length }} - {{ AlternateText }}</span>' +
	'  <div class="ng-thumbnails-wrapper">' +
	'    <div class="ng-thumbnails slide-left">' +
	'      <div ng-repeat="i in images">' + 
	'        <img ng-src="{{ i.ImageUrl }}" ng-class="{\'active\': index === $index}" ng-click="changeImage($index)" />' +
	'      </div>' +
	'    </div>' +
	'  </div>' +
	'</div>'
	...
	return {
		...
		link: function (scope, element, attrs) {
			...
			var loadImage = function (i) {
				...
				image.src = scope.images[i].FullSizeImageUrl;
				scope.loading = true;
			...
			var showImage = function (i) {
				loadImage(scope.index).then(function(resp) {
				    scope.FullSizeImageUrl = resp.src;
					smartScroll(scope.index);
				});
				scope.description = scope.images[i].AlternateText || '';
			};
			...
			var calculateThumbsWidth = function () {
				...
				angular.forEach($thumbnails.find('img'), function(thumb) {
					...
					width += 15; // margin-right
					visible_width = thumb.clientWidth + 0;
				});
				return {
					width: width,

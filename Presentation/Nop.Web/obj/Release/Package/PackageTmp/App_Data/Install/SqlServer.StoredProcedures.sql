...
CREATE PROCEDURE [dbo].[PlanLoadAllPaged]
(
	@CategoryIds		nvarchar(MAX) = null,	--a list of category IDs (comma-separated list). e.g. 1,2,3
	@ManufacturerId		int = 0,
	@StoreId			int = 0,
	@VendorId			int = 0,
	@WarehouseId		int = 0,
	@PlanTypeId		int = null, --plan type identifier, null - load all plans
	@VisibleIndividuallyOnly bit = 0, 	--0 - load all plans , 1 - "visible indivially" only
	@MarkedAsNewOnly	bit = 0, 	--0 - load all plans , 1 - "marked as new" only
	@PlanTagId		int = 0,
	@FeaturedPlans	bit = null,	--0 featured only , 1 not featured only, null - load all plans
	@PriceMin			decimal(18, 4) = null,
	@PriceMax			decimal(18, 4) = null,
	@Keywords			nvarchar(4000) = null,
	@SearchDescriptions bit = 0, --a value indicating whether to search by a specified "keyword" in plan descriptions
	@SearchSku			bit = 0, --a value indicating whether to search by a specified "keyword" in plan SKU
	@SearchPlanTags  bit = 0, --a value indicating whether to search by a specified "keyword" in plan tags
	@UseFullTextSearch  bit = 0,
	@FullTextMode		int = 0, --0 - using CONTAINS with <prefix_term>, 5 - using CONTAINS and OR with <prefix_term>, 10 - using CONTAINS and AND with <prefix_term>
	@FilteredSpecs		nvarchar(MAX) = null,	--filter by specification attribute options (comma-separated list of IDs). e.g. 14,15,16
	@LanguageId			int = 0,
	@OrderBy			int = 0, --0 - position, 5 - Name: A to Z, 6 - Name: Z to A, 10 - Price: Low to High, 11 - Price: High to Low, 15 - creation date
	@AllowedCustomerRoleIds	nvarchar(MAX) = null,	--a list of customer role IDs (comma-separated list) for which a plan should be shown (if a subjet to ACL)
	@PageIndex			int = 0, 
	@PageSize			int = 2147483644,
	@ShowHidden			bit = 0,
	@OverridePublished	bit = null, --null - process "Published" property according to "showHidden" parameter, true - load only "Published" plans, false - load only "Unpublished" plans
	@LoadFilterableSpecificationAttributeOptionIds bit = 0, --a value indicating whether we should load the specification attribute option identifiers applied to loaded plans (all pages)
	@FilterableSpecificationAttributeOptionIds nvarchar(MAX) = null OUTPUT, --the specification attribute option identifiers applied to loaded plans (all pages). returned as a comma separated list of identifiers
	@TotalRecords		int = null OUTPUT
)
AS
BEGIN
	
	/* Plans that filtered by keywords */
	CREATE TABLE #KeywordPlans
	(
		[PlanId] int NOT NULL
	)
	...
	SET @sql = '
	INSERT INTO #DisplayOrderTmp ([PlanId])
	SELECT p.Id
	FROM
		dbo.[Plan] p with (NOLOCK)'
	
	IF @CategoryIdsCount > 0
	BEGIN
		SET @sql = @sql + '
		LEFT JOIN Plan_Category_Mapping pcm with (NOLOCK)
			ON p.Id = pcm.PlanId'
	END
	...
	BEGIN
		...
		IF @FeaturedPlans IS NOT NULL
		BEGIN
			SET @sql = @sql + '
		AND pcm.IsFeaturedPlan = ' + CAST(@FeaturedPlans AS nvarchar(max))
		END
	END
	...
	--filter by plan type
	IF @PlanTypeId is not null
	BEGIN
		SET @sql = @sql + '
		AND p.PlanTypeId = ' + CAST(@PlanTypeId AS nvarchar(max))
	END
		...
		AND (p.SubjectToAcl = 0 OR EXISTS (
			...
			WHERE
				[fcr].CustomerRoleId IN (
					SELECT [acl].CustomerRoleId
					FROM [AclRecord] acl with (NOLOCK)
					WHERE [acl].EntityId = p.Id AND [acl].EntityName = ''Plan''
				)
			))'
	END
	...
	BEGIN
		...
		AND (p.LimitedToStores = 0 OR EXISTS (
			SELECT 1 FROM [StoreMapping] sm with (NOLOCK)
			WHERE [sm].EntityId = p.Id AND [sm].EntityName = ''Plan'' and [sm].StoreId=' + CAST(@StoreId AS nvarchar(max)) + '
			))'
	END
	...
	FROM
		#PageIndex [pi]
		INNER JOIN dbo.[Plan] p with (NOLOCK) on p.Id = [pi].[PlanId]
	WHERE
		[pi].IndexId > @PageLowerBound AND 
		[pi].IndexId < @PageUpperBound
	ORDER BY
...
BEGIN
	...
	AND
	--shopping cart items
	((@OnlyWithoutBorrowCart=0) OR (NOT EXISTS(SELECT 1 FROM [BorrowCartItem] sci with (NOLOCK) inner join [Customer] with (NOLOCK) on sci.[CustomerId]=c.[Id])))
	AND
	--guests only
